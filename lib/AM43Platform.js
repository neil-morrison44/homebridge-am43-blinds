...
class AM43Platform {
  constructor(log, config, api) {
    ...
    api.on("didFinishLaunching", () => {
      this.scanForDevices()
      setInterval(() => this.disconnectUnusedDevices(), 20e3)
    })
  ...
  disconnectUnusedDevices() {
    const interactionTimeout =
      (this.configJSON[CONFIG_KEY_HAP_INTERACTION_TIMEOUT] ||
        DEFAULT_HAP_INTERACTION_TIMEOUT) * 1000

    this.accessories
      .filter(({ am43device }) => am43device && am43device.isConnected)
      .forEach((accessory) => {
        const timeSinceInteraction =
          Date.now() - accessory.am43device.lastCommandTimestamp
        this.log("timeSinceInteraction " + timeSinceInteraction)
        if (
          interactionTimeout > 0 &&
          timeSinceInteraction > interactionTimeout
        ) {
  ...
  configureDeviceOnAccessory(accessory, device) {
    ...
    window.setTimeout(() => accessory.updateInformation(), Math.random() * 10e3)
  }
