...
const { AM43_SERVICE_ID } = require("./variables/device")
...
class AM43Platform {
  constructor(log, config, api) {
    ...
    api.on("didFinishLaunching", () => this.didFinishedLaunching())
  ...
  configureDeviceOnAccessory(accessory, device) {
    ...
    device.on("direction", (direction) => {
      ...
      this.log.info("direction", direction, target)
    ...
    device.on("targetPosition", (position) => {
      var targetPosition =
        position == null ? position : accessory.am43device.position
      this.log.debug(
        "Notifying of new target position: " + (100 - targetPosition)
      )
      this.log.info("targetPosition", targetPosition)
      accessory.windowCoveringService
        .getCharacteristic(this.api.hap.Characteristic.TargetPosition)
    ...
    device.on("position", (position) => {
      ...
      this.log.info("position", position, device.direction)
      accessory.windowCoveringService
        .getCharacteristic(this.api.hap.Characteristic.CurrentPosition)
  ...
  configureWindowCoveringServiceOnAccessory(accessory) {
    ...
    service
      ...
      .on("get", (callback) => {
        if (!accessory.am43device) {
          this.startScanningForDevices(MISSING_DEVICES_SCANNING_TIMEOUT)
          callback("No device found please try again", null)
          return
        }
