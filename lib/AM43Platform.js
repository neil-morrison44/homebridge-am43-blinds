  ...
  configurePropertiesOnAccessory(accessory) {
    accessory.hapInteractionTimeout =
      this.configJSON[CONFIG_KEY_HAP_INTERACTION_TIMEOUT] != undefined
        ? this.configJSON[CONFIG_KEY_HAP_INTERACTION_TIMEOUT]
        : DEFAULT_HAP_INTERACTION_TIMEOUT
    accessory.context.lastHAPInteraction = null // The last time the homekit accessory procotol tried to interact with the device, this is used to disconnect the device to conserve power.
    accessory.secondsSinceLastHAPInteraction = () => {
      return accessory.context.lastHAPInteraction
        ? Math.floor((Date.now() - accessory.context.lastHAPInteraction) / 1000)
  ...
  configureWindowCoveringServiceOnAccessory(accessory) {
    ...
    service
      ...
      .on("get", (callback) => {
        ...
        accessory.context.lastHAPInteraction = Date.now()
