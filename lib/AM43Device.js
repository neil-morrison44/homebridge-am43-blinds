...
class AM43Device extends EventEmitter {
  ...
  async reconnect() {
    ...
    this.connectingPromise = await retry(
      async () => {
  ...
  async serviceDiscoveryAsync() {
    ...
    this.discoveringPromise = await this.peripheral.discoverSomeServicesAndCharacteristicsAsync(
      [AM43_SERVICE_ID],
      [AM43_CHARACTERISTIC_ID]
    )
    const { characteristics } = this.discoveringPromise
